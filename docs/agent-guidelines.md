...
- **A state's own price must come from a feed that reports that state's own odds — but national and out-of-state odds may be collected and shown, so long as every reader can see what they are.** A national page, a Las Vegas column, or another state's licence is never a substitute for the state's price; it is context, and each surface must label it as such ("not reachable from {ST}") rather than presenting it as the state's own. On the output side the rule is `coverage.locality_marking` with `coverage.locality_applies` deciding which runs it governs — every surface asks those two rather than spelling the condition itself, labels each unreachable leg, and reports how many positions have no local leg at all.
